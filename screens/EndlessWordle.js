import { StyleSheet, Text, View, SafeAreaView,ScrollView, LogBox, Alert, ActivityIndicator, TouchableOpacity, Image, Button} from 'react-native'
...
const EndlessWordle = () => {
  ...
  const { dailyWord, isLoading, refreshWord } = useDailyWord();
  ...
  useEffect(() => {
    const checkGameStateAndRefresh = async () => {
      if (gameSate === 'won') {
        // Automatically refresh the game if it has ended
        await refreshGame();
      }
    };
  
    checkGameStateAndRefresh();
  }, [gameSate, refreshGame]); // Depend on gameSate and refreshGame function



  const refreshGame = async () => {
    await refreshWord(); // Fetch a new word from your daily word source
  
    // Reset game state for the new word
    const newWord = dailyWord.split(""); // Assuming dailyWord has been updated by refreshWord()
    const initializedRows = new Array(Number_Of_Tries).fill(new Array(newWord.length).fill(''));
    
    setRows(initializedRows);
    setCurRow(0);
    setCurCol(0);
    setGameState('playing');
    setLetters(newWord); // Update letters state with the new word's letters
...
if (!isLoading && !dailyWord) {
  ...
  if (gameSate != 'playing') {
    return (<EndlessEndScreen won={gameSate == 'won'} rows={rows} getCellBGColor={getCellBGColor} navigation={navigation}/>)
  }
