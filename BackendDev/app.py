...
# List of climate change terms for filtering 
climate_terms = [
...
def preprocess(text): #preprocessing text for analysis
    doc = nlp(text.lower())
...
def fetch_and_scrape_article(url):
    try:
        ...
        if response.status_code == 200:
            ...
            selectors = [ #html selectors used for web scraping to select words 
                {'tag': 'article'},
...
def fetch_news_articles(api_key):
    ...
    if response.status_code == 200:
        ...
        return []
    # this gets the link from the climate change api 
...
#Extracting single words using TF-IDF and selecting the to 360 terms
...
# using the first news api for fetch articles 
...
def climate_news():
    ...
    if not articles_info:
        return jsonify({"error": "No articles found or could not fetch/scrape articles."}), 404 # error produced if no articles are produced
